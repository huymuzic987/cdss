...
export function RegimenDecisionEditor({ open, result, presentation, summary, alertTitle, confirmedFindings, detailedFindings, baselineClinicalPlan, locale, onBack, onSaved }: RegimenDecisionEditorProps) {
  ...
  const [options, setOptions] = useState<FinalRegimenOption[]>(() => presentation.regimenOptions.length > 0 ? presentation.regimenOptions.map((option) => ({ ...option, components: option.components.map((component) => ({ ...component })) })) : [{ id: 'regimen-option-1', components: [] }])
  ...
  const toggleReason = (code: RejectionReasonCode) => { setReasons((current) => current.includes(code) ? current.filter((item) => item !== code) : [...current, code]); setError(undefined) }
  const requestSave = () => {
    setError(undefined)
    if (reasons.length === 0) return setError(vi ? 'Chọn ít nhất một lý do từ chối.' : 'Select at least one rejection reason.')
    if (hasOther && wordCount(otherReason) < 50) return setError(vi ? 'Lý do khác phải có ít nhất 50 từ.' : 'The Other reason must contain at least 50 words.')
    setComparisonOpen(true)
  }
  const confirmSave = async () => {
    setSaving(true); setError(undefined)
    const payload: RegimenDecisionCreateRequest = { outcome: 'rejected', evaluation_snapshot: result as unknown as JsonObject, baseline: { clinical_plan: baselineClinicalPlan, regimen_options: selections(baselineOptions) }, final: { clinical_plan: planItems, regimen_options: selections(options) }, rejection_reasons: reasons, ...(otherReason.trim() ? { other_rejection_reason: otherReason.trim() } : {}) }
    try { await saveRegimenDecision(payload); setComparisonOpen(false); onSaved() } catch (reason: unknown) { setError(reason instanceof Error ? reason.message : (vi ? 'Không thể lưu phác đồ.' : 'Unable to save regimen.')) } finally { setSaving(false) }
  }
  if (!open) return null
  return (
    <div className="cds-editor-overlay" onMouseDown={(event) => event.target === event.currentTarget && onBack()}>
      <div className="cds-editor-modal" role="dialog" aria-modal="true" aria-label={vi ? 'Chỉnh sửa phác đồ' : 'Edit regimen'}>
        ...
        <div className="cds-editor-body">
          ...
          <ClinicalSection title={vi ? 'Kế hoạch lâm sàng' : 'Clinical Plan'} action={<button type="button" className="cds-section-plus" onClick={() => setPlanSidebarOpen(true)}>+</button>}><div className="cds-plan-rows">{planItems.length === 0 && <p className="cds-empty">{vi ? 'Chưa có kế hoạch.' : 'No clinical-plan rows yet.'}</p>}{planItems.map((item, index) => <div className="cds-plan-row" key={`${item.type}-${index}`}><span>{planLabel(item, locale)}</span><button type="button" onClick={() => setPlanItems((current) => current.filter((_, rowIndex) => rowIndex !== index))}>×</button></div>)}</div><ClinicalPlanSidebar open={planSidebarOpen} locale={locale} onCancel={() => setPlanSidebarOpen(false)} onAdd={(item) => { setPlanItems((current) => [...current, item]); setPlanSidebarOpen(false) }} /></ClinicalSection>
          <ClinicalSection title={vi ? 'Phác đồ thuốc cuối cùng' : 'Final drug regimen'} action={<button type="button" className="cds-section-plus" onClick={() => setCatalogOpen(true)}>+</button>}><EditableRegimen locale={locale} options={options} catalog={catalog} catalogOpen={catalogOpen} catalogLoading={catalogLoading} catalogError={catalogError} onCatalogOpenChange={setCatalogOpen} onChange={setOptions} /></ClinicalSection>
          <ClinicalSection title={vi ? 'Lý do từ chối' : 'Reason for rejecting'}><div className="cds-rejection-reasons">{reasonCodes.map((reason) => <label key={reason.code}><input type="checkbox" checked={reasons.includes(reason.code)} onChange={() => toggleReason(reason.code)} />{vi ? reason.vi : reason.en}</label>)}</div>{hasOther && <label className="cds-editor-textarea-label">{vi ? 'Mô tả lý do khác (tối thiểu 50 từ)' : 'Describe the Other reason (minimum 50 words)'}<textarea rows={5} value={otherReason} onChange={(event) => setOtherReason(event.target.value)} /></label>}</ClinicalSection>
